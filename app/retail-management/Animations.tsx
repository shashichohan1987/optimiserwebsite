...
export default function Animations() {
  useEffect(() => {
    ...
    async function init() {
      ...
      /* ---- interactive showcase rotator (page inline script, no-op if absent) ---- */
      (() => {
      ...
      /* ---- sub-nav scroll-spy (page inline script, no-op if absent) ---- */
      (() => {
      ...
      /* ---- orb-core icon fallback (was inline onerror in source markup) ---- */
      (() => {
        const icon = document.querySelector<HTMLImageElement>(".orb-core .knot img");
        if (!icon) return;
        const onErr = () => {
          if (icon.parentElement) icon.parentElement.textContent = "O";
        };
        icon.addEventListener("error", onErr);
        cleanups.push(() => icon.removeEventListener("error", onErr));
      })();
