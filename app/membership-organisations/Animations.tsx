...
export default function Animations() {
  useEffect(() => {
    ...
    async function init() {
      ...
      /* ---- orb-core knot image fallback (page inline onerror) ---- */
      (() => {
        const img = document.getElementById("orbKnotImg") as HTMLImageElement | null;
        if (!img) return;
        const onError = () => {
          const parent = img.parentElement;
          if (parent) parent.textContent = "O";
        };
      ...
      const TILT = ".cb-h,.cb-c2,.cb-pillar";
      ...
      const nums = document.querySelectorAll<HTMLElement>(".cb-stat b,[data-count]");
      ...
      /* ---- motion.js: scroll parallax on product shot + floating chips ---- */
      ctx = gsap.context(() => {
        gsap.utils.toArray<HTMLElement>(".hero-shot").forEach((el) => {
          gsap.fromTo(
            el,
            { y: 34 },
            {
