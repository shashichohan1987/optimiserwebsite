...
export default function Animations() {
  useEffect(() => {
    ...
    async function init() {
      ...
      /* ---- integrations hub icon fallback (page inline onerror) ---- */
      (() => {
        const hubImg = document.querySelector<HTMLImageElement>(".ig-hub img");
        if (!hubImg) return;
        const onError = () => {
          if (hubImg.parentElement) hubImg.parentElement.textContent = "O";
        };
        hubImg.addEventListener("error", onError);
        cleanups.push(() => hubImg.removeEventListener("error", onError));
      })();
