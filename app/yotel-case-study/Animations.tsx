...
/* Page-specific behaviors ported from optimiser-site/motion.js.
   ...
   handled by components/Nav.tsx. This page has no interactive rotator
   or sub-nav scroll-spy in its source markup, so only the generic
   motion.js effects (tilt, stat counters, parallax) are ported here. */
...
export default function Animations() {
  useEffect(() => {
    ...
    async function init() {
      ...
      const TILT = ".vcard,.t2-b,.cb-h,.cb-c2,.ind,.t2-stat,.cb-pillar,.pcard,.fcard,.bcard,.side-card,.rstat";
      ...
      /* .rstat b holds this page's result-metric counters (360°, 1, Faster) */
      const nums = document.querySelectorAll<HTMLElement>(
        ".t2-stat b,.cb-stat b,.ms-stat .lb b,.rstat b,[data-count]"
      );
