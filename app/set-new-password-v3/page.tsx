...
export default function SetNewPasswordV3Page() {
  // Theme
  ...
  const [pw1Show, setPw1Show] = useState(false);
  ...
  const cols = ['#FF6F5B', '#FFAA2B', '#00A1DF', '#19BE77'];
  const words = ['Weak', 'Fair', 'Good', 'Strong'];
  const s = strength(pw1);
  const hintDefault = 'At least 8 characters, with upper & lower case, a number, and a special character.';
  const hintText = pw1
    ? s === 4
      ? 'Strong password ✓'
      : words[Math.max(0, s - 1)] + ' — at least 8 characters, with upper & lower case, a number, and a special character.'
    : hintDefault;

  const handlePw1Input = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setPw1(v);
    if (pw1Err && strength(v) === 4) setPw1Err(false);
  };

  const handlePw2Input = (e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setPw2(v);
    if (pw2Err && v === pw1) setPw2Err(false);
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const ok1 = strength(pw1) === 4;
    const ok2 = pw2 === pw1 && pw2.length > 0;
    setPw1Err(!ok1);
    setPw2Err(!ok2);
    if (!ok1 || !ok2) {
      setFormShake(true);
      setTimeout(() => setFormShake(false), 420);
      (!ok1 ? pw1Ref : pw2Ref).current?.focus();
      return;
    }
    setSubmitLabel('Setting password…');
    setSubmitOpacity('.7');
    setSubmitDisabled(true);
    setTimeout(() => {
      setSubmitLabel('Password set ✓');
      setSubmitOpacity('1');
      setSubText('Your password has been updated. Taking you to sign in…');
      setTimeout(() => {
      ...
      <div className="auth">
        ...
        <main className="panel">
          ...
          <div className="form-wrap">
            <form className={`card rv${cardIn ? ' in' : ''}${formShake ? ' shake' : ''}`} id="pwdForm" noValidate onSubmit={handleSubmit}>
              <span className="eyebrow">Account recovery</span>
              <h1>Set a new password</h1>
              <p className="sub">{subText ?? <>Choose a new password for <b style={{ color: 'var(--navy)' }}>oliver.bennett@optimiser.com</b>.</>}</p>

              <div className={`field${pw1Err ? ' err' : ''}`} id="f-pw1">
                <label htmlFor="pw1">New password</label>
                <div className="field-wrap">
                  <span className="lead"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="11" width="18" height="11" rx="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg></span>
                  <input
                    ref={pw1Ref}
                    id="pw1"
                    name="new-password"
                    ...
                    required
                    value={pw1}
                    onChange={handlePw1Input}
                  />
                  <button
                    type="button"
                    className="peek"
                    id="peek1"
                    aria-label={pw1Show ? 'Hide password' : 'Show password'}
                    onClick={() => setPw1Show((v) => !v)}
                  ><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg></button>
                </div>
                <div className="pw-meter" id="pwMeter" aria-hidden="true">
                  {[0, 1, 2, 3].map((k) => (
                    <i key={k} style={pw1 && k < s ? { background: cols[s - 1] } : undefined}></i>
                  ))}
                </div>
                <p className="pw-hint" id="pwHint" style={pw1 && s === 4 ? { color: '#0B7A4C' } : undefined}>{hintText}</p>
                <span className="msg" role="alert" id="pw1Msg">Password doesn&apos;t meet the requirements</span>
              </div>

              <div className={`field${pw2Err ? ' err' : ''}`} id="f-pw2">
                ...
                <div className="field-wrap">
                  ...
                  <input
                    ...
                    required
                    value={pw2}
                    onChange={handlePw2Input}
                  />
                  <button
                    type="button"
                    className="peek"
                    id="peek2"
                    aria-label={pw2Show ? 'Hide password' : 'Show password'}
                    onClick={() => setPw2Show((v) => !v)}
                  ><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg></button>
                </div>
                <span className="msg" role="alert">Passwords don&apos;t match</span>
              </div>
              ...
              <button
                ...
                style={{ marginTop: '10px', ...(submitOpacity !== undefined ? { opacity: submitOpacity } : {}) }}
              >
                {submitLabel ?? <>Set password <span className="arr">→</span></>}
              </button>
              ...
              <p className="alt-row"><a href="/login-v3">← Back to sign in</a></p>
            </form>
          </div>
