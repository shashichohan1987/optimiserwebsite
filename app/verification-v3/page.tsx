...
export default function VerificationV3Page() {
  // Theme
  ...
  const [verSub, setVerSub] = useState<string | null>(null);
  const [resendWaiting, setResendWaiting] = useState(false);
  const [resendN, setResendN] = useState(30);

  const boxRefs = useRef<(HTMLInputElement | null)[]>([]);

  const handleBoxInput = (k: number, e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value.replace(/\D/g, '').slice(0, 1);
    const next = [...digits];
    next[k] = v;
    setDigits(next);
    if (v && k < 5) boxRefs.current[k + 1]?.focus();
    if (codeErr && next.every((x) => x)) setCodeErr(false);
  };

  const handleBoxKeyDown = (k: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace' && !digits[k] && k > 0) {
      boxRefs.current[k - 1]?.focus();
      const next = [...digits];
      next[k - 1] = '';
      setDigits(next);
      e.preventDefault();
    }
    if (e.key === 'ArrowLeft' && k > 0) boxRefs.current[k - 1]?.focus();
    if (e.key === 'ArrowRight' && k < 5) boxRefs.current[k + 1]?.focus();
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const ok = digits.every((b) => b);
    setCodeErr(!ok);
    if (!ok) {
      setFormShake(true);
      setTimeout(() => setFormShake(false), 420);
      const firstEmpty = digits.findIndex((b) => !b);
      if (firstEmpty >= 0) boxRefs.current[firstEmpty]?.focus();
      return;
    }
    ...
    setTimeout(() => {
      setSubmitLabel('Verified ✓');
      setSubmitOpacity('1');
      setVerSub('Code verified. Taking you to your workspace…');
      setTimeout(() => {
        window.location.href = '/my-apps-v3';
      }, 900);
    }, 1200);
  };
      ...
      <div className="auth">
        ...
        <main className="panel">
          ...
          <div className="form-wrap">
            <form className={`card rv${cardIn ? ' in' : ''}${formShake ? ' shake' : ''}`} id="verForm" noValidate onSubmit={handleSubmit}>
              <span className="eyebrow">Account security</span>
              <h1>Verify it&apos;s you</h1>
              <p className="sub" id="verSub">{verSub ?? <>We sent a verification code to <b style={{ color: 'var(--navy)' }}>oliver.bennett@optimiser.com</b></>}</p>

              <div className={`field${codeErr ? ' err' : ''}`} id="f-code">
                <label htmlFor="c1">Verification code</label>
                <div className="code-row" id="codeRow">
                  {digits.map((d, k) => (
                    <input
                      key={k}
                      ref={(el) => {
                        boxRefs.current[k] = el;
                      }}
                      id={k === 0 ? 'c1' : undefined}
                      className={`code-box${d ? ' filled' : ''}`}
                      type="text"
                      inputMode="numeric"
                      autoComplete={k === 0 ? 'one-time-code' : undefined}
                      maxLength={1}
                      aria-label={`Digit ${k + 1}`}
                      value={d}
                      onChange={(e) => handleBoxInput(k, e)}
                      onKeyDown={(e) => handleBoxKeyDown(k, e)}
                      onPaste={handleBoxPaste}
                    />
                  ))}
                </div>
                <span className="msg" role="alert">Please enter the 6-digit code</span>
              </div>
              ...
              >
                {submitLabel ?? <>Verify <span className="arr">→</span></>}
              </button>

              <p className="alt-row" style={{ marginTop: '20px' }}>
                Didn&apos;t get a code?{' '}
                <a href="#" id="resendLink" style={resendWaiting ? { display: 'none' } : undefined} onClick={handleResend}>Resend</a>
                <span id="resendWait" style={{ display: resendWaiting ? 'inline' : 'none', color: 'var(--mut)' }}>Resend in <b id="resendN">{resendN}</b>s</span>
              </p>
