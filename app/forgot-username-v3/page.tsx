...
export default function ForgotUsernameV3Page() {
  // Theme
  ...
  const [subText, setSubText] = useState('To recover your username, enter your registered Optimiser email address.');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setSubmitLabel('Sending username…');
    setSubmitOpacity('.7');
    setSubmitDisabled(true);
    setTimeout(() => {
      setSubmitLabel('Username sent ✓');
      setSubmitOpacity('1');
      setSubText('If that email is registered, your username is on its way to your inbox.');
    }, 1400);
  };
          ...
          <div className="form-wrap">
            <form className={`card rv${cardIn ? ' in' : ''}${formShake ? ' shake' : ''}`} id="recForm" noValidate onSubmit={handleSubmit}>
              ...
              <div className={`field${recError ? ' err' : ''}`} id="f-rec">
                ...
                <span className="msg" role="alert">Please enter your email address</span>
              </div>
              ...
              >
                {submitLabel ?? <>Recover username <span className="arr">→</span></>}
              </button>
              ...
              <p className="alt-row"><a href="/login-v3">← Back to log in</a></p>
            </form>
          </div>
